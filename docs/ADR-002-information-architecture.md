...
|----------|--------|----------|
| **What is true right now?** | Knowledge Base (SQLite + BRIEF.md) | "Alice left", "Bob: 10h/week", "Feature X: removed" |
| **What should I do today?** | Konban (Notion Kanban) | "Contact institutes", "Send proposal to Client C" |
| **What's the EO strategy?** | Notion Brain (Active Context) | "Priority #1: therapist acquisition via institutes" |
| **What features are we building?** | Linear | "PROJ-123: Implement mandatory onboarding flow" |
...
- "Bob's role is Strategy & Support" → KB fact
- "Alice left" → KB fact (supersedes "Alice is QA Tester")
- "Feature X removed" → KB decision
...
- "Get testimonials from Luisa" → Konban task
...
- "We're removing Feature X to simplify the platform" → Brain (Active Context decision section)
...
- "Build mandatory onboarding flow with 30min scheduling" → Linear (PROJ-xxx)
- "Remove Feature X UI from platform" → Linear

### Records and deliverables → Google Drive

If it's a document someone else will read, or needs long-term archival.

- "Client C notary proposal" → GDrive
- "Client D phase 2 scope" → GDrive
- "Bob's EO role adjustment email" → GDrive (if worth archiving)
...
- "Use worktrees for parallel EO sessions" → MEMORY.md
...
- **Brain decisions** are curated by Jane during Kraken sessions. They include strategic context: "Why this matters for the roadmap."

The KB is the **ledger** (complete, automatic). The Brain is the **executive summary** (curated, contextual).

### MEMORY.md shrinks over time

Before the KB, MEMORY.md stored facts about people and projects (Oz's behavioral history, Alice's role, Bob's therapist friend). Those migrate to the KB. MEMORY.md keeps only tool-usage patterns and workflow gotchas.
...
- Entity name index: `~/.knowledge-base/entity-index.json` (rebuilt by daemon)
- Debug log: `~/.knowledge-base/recall-debug.log`
